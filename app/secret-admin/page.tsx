        ...
        <div className="min-h-screen flex items-center justify-center p-4">
            ...
            >
                <div className="mb-8 text-center">
                    <div className="text-[10px] uppercase tracking-[0.4em] font-bold mb-3" style={{ color: accent }}>
                        Admin Portal
                    </div>
                ...
                >
                    <input
                        ...
                        className={`w-full dark:bg-white/5 bg-black/5 border px-5 py-4 dark:text-white text-black text-sm outline-none transition-all duration-300 placeholder:opacity-30 rounded-xl ${error
                            ? "border-red-500/50"
...
function Toast({ message, type }: { message: string; type: "success" | "error" }) {
    return (
        <motion.div
            ...
            className={`fixed top-6 left-1/2 z-50 px-5 py-3 text-sm font-semibold shadow-xl rounded-full ${type === "success"
...
function EventForm({
    ...
    const labelStyle = "block text-[10px] uppercase tracking-widest dark:text-white/40 text-black/40 mb-2 font-bold";
    const inputStyle = "w-full dark:bg-white/5 bg-black/5 border dark:border-white/10 border-black/10 px-4 py-3 dark:text-white text-black text-sm outline-none focus:border-opacity-100 transition-all duration-300 rounded-lg";
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                    ...
                    <input
                        ...
                        style={{ borderLeft: form.title ? `3px solid ${accent}` : undefined }}
                    />
                </div>
                <div>
                ...
                <div className="md:col-span-2 flex items-center justify-between p-4 dark:bg-white/5 bg-black/5">
                    <div>
                        <span className="text-sm font-bold block dark:text-white text-black">Timeline Status</span>
                        <span className="text-[10px] uppercase tracking-wider opacity-40">{form.is_upcoming ? "Live/Future Event" : "Historical Event"}</span>
                    </div>
                    <button
                        type="button"
                    ...
                    >
                        <span className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-500 shadow-md ${form.is_upcoming ? "right-1" : "left-1"}`} />
                    </button>
                </div>
            </div>
...
// ─── Event Row ───────────────────────────────────────────────────────────────
function EventRow({ event, onEdit, onDelete }: { event: Event; onEdit: () => void; onDelete: () => void }) {
    const { resolvedTheme } = useTheme();
    const accent = resolvedTheme === "dark" ? "#FA0001" : "#0DA5F0";
    ...
    return (
        ...
        >
            <div className="relative w-20 h-14 hidden sm:block overflow-hidden bg-black/5 dark:bg-white/5 rounded-lg">
                {event.cover_image ? (
                    <Image src={event.cover_image} alt="" fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                ) : (
            ...
            <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                    <h3 className="text-sm font-black dark:text-white text-black truncate max-w-[200px] sm:max-w-none tracking-tighter">{event.title}</h3>
                    <span
                    ...
                    {event.registration_link && (
                        <span className="text-[9px] text-emerald-500 font-black tracking-widest flex items-center gap-1">
                            <span className="w-1 h-1 rounded-full bg-emerald-500" /> Past Event
                        </span>
                    )}
                </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] dark:text-white/40 text-black/40 font-bold uppercase tracking-wider">
                    <span className="flex items-center gap-1"><Calendar size={10} /> {new Date(event.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                    <span className="flex items-center gap-1 sm:hidden"><MapPin size={10} /> {event.venue.substring(0, 15)}...</span>
                    <span className="hidden sm:flex items-center gap-1"><MapPin size={10} /> {event.venue}</span>
                </div>
            </div>
        ...
        <div className="min-h-screen dark:bg-[oklch(0.145_0_0)] bg-white transition-colors duration-500">
            ...
            <div className="max-w-6xl mx-auto px-4 pt-40 pb-20">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-16">
                    <div>
                        <div className="text-[10px] uppercase tracking-[0.4em] font-black opacity-40 mb-2 dark:text-white text-black">System Terminal</div>
                        <h1 className="text-5xl font-black tracking-tighter dark:text-white text-black uppercase">Event <span style={{ color: accent }}>Manager</span></h1>
                    </div>
                    {!showForm && (
                        <button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    {/* Event List Section */}
                    <div className={`${showForm ? "lg:col-span-12" : "lg:col-span-12"} space-y-4`}>
                        <AnimatePresence mode="wait">
                            {showForm ? (
                                ...
                                <div key="list" className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {loading ? (
                                        [1, 2, 3, 4].map(i => <div key={i} className="h-32 dark:bg-white/5 bg-black/5 animate-pulse" />)
                                    ) : (
                                        <>
                                            {events.length === 0 ? (
                                                <div className="col-span-full py-20 text-center dark:text-white/20 text-black/20 font-black uppercase tracking-widest">No Records Found</div>
                                            ) : (
                                                events.map(event => (
                                                    <EventRow
                                                        key={event._id}
                                                        event={event}
                                                        onEdit={() => { setEditingEvent(event); setShowForm(true); window.scrollTo({ top: 0, behavior: "smooth" }); }}
